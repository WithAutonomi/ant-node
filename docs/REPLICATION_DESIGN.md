...
22. On every `NeighborSyncCycleComplete(self)`, node MUST run a prune pass over all stored records and `PaidForList` entries using the current local routing table. For stored records, retention responsibility is evaluated at the storage-retention width `CLOSE_GROUP_SIZE + STORAGE_ADMISSION_MARGIN` (9 at reference parameters):
    ...
    - Audit scheduling groups a bounded rotating candidate window by target peer and sends multi-key requests, split at `max(floor(sqrt(local_stored_keys)), 1)` keys per request. The pass is bounded by actual requests and complete candidate proof sets, not candidate-to-peer edges; no partial candidate audit is carried across passes. Deployed oversized-batch rejections are split and retried without changing the wire format or penalising responder capacity.
