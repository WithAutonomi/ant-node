...
- `RepairProof(P, K)`: local evidence that node `N` sent peer `P` a replica repair hint for key `K` while `P` was in `N`'s self-inclusive `CloseGroup(K)`. The proof records the close-group snapshot and local neighbor-sync cycle epoch at hint time; if the current self-inclusive close group for `K` differs, all proofs for `K` are invalidated. If `P` leaves `LocalRT(self)`, all proofs for `P` are invalidated. Peers require a fresh hint after re-entry.
- `RepairOpportunity(P, KSet)`: for every key in `KSet`, a mature `RepairProof(P, K)` exists and peer `P` has had at least one subsequent local neighbor-sync cycle epoch to repair before audit evaluation. Repair proofs are retained per key only for one close-group snapshot, bounding memory to `local_key_count * CLOSE_GROUP_SIZE`.
...
22. On every `NeighborSyncCycleComplete(self)`, node MUST run a prune pass over all stored records and `PaidForList` entries using the current local routing table. For stored records, retention responsibility is evaluated at the storage-retention width `CLOSE_GROUP_SIZE + STORAGE_ADMISSION_MARGIN` (9 at reference parameters):
    ...
    - Candidates are processed subject to operational scheduling: bootstrap state (rule 19), the bounded per-pass candidate/request budgets, and a retained answerable commitment may defer work or veto deletion. These are deferrals only — they never remove candidate status or restart hysteresis.
    - A mature record outside a COMPLETE widest-admission lookup (`paid_list_close_group_size`, 20 at reference parameters) is a fast-delete candidate: no remote possession audit is required. Immediately before deletion, revalidate that bootstrap is drained, hysteresis is still mature, self remains outside both the retention width and a complete width-20 group, and no answerable retained commitment holds the key. This explicitly accepts that distance does not prove closer replicas possess the bytes.
    - Other mature candidates use direct current-close-group possession confirmation: challenge the current strict `CloseGroup(K)` (taken directly from the local routing table, never filtered through `RepairProofs` or prior neighbor-sync hints) with the nonce-bound audit proof, and delete only when all but one of the current close group (6 of 7 at reference parameters) return valid positive proofs. Missing, timed-out, bootstrapping, malformed, absent, and digest-mismatching responses never count positively. Below the threshold, retain the record, timestamp, and candidacy for a later pass.
    - Audit scheduling groups a bounded rotating candidate window by target peer and sends multi-key requests, split at `max(floor(sqrt(local_stored_keys)), 1)` keys per request. The pass is bounded by actual requests and complete candidate proof sets, not candidate-to-peer edges; no partial candidate audit is carried across passes. A responder's size rejection is not renegotiated in-band: since its acceptance limit already carries a 2x margin over the sender size, an honestly-sized peer rejects only once close-group store spread exceeds 4x, so under the assumed ~2x spread a size rejection is graded as an attributable failure (distinct `size_reject` label), never a possession signal.
    - Immediately before audited deletion, revalidate against the current routing table: the key must still be outside the retention width, must not be held by an answerable retained commitment, and the positive reports must still satisfy the CURRENT strict close group (membership churn invalidates stale reports). If the key has meanwhile moved outside a complete width-20 group, the fast path may delete it after full fast-path revalidation.
...
- **Prune scheduling**: bootstrap drain, bounded local fast deletes, bounded audited candidates/requests, and retained commitments govern execution. Deferral never removes candidacy or restarts hysteresis.
- **Far-out prune deletion**: a mature candidate outside a complete widest-admission group (20) is deleted without a remote proof after immediate local revalidation; the accepted trade-off is that local closeness does not prove remote possession.
- **Near-out prune deletion confirmation**: other mature candidates use a direct possession check of the current strict `CloseGroup(K)` (7), requiring all-but-one (6 of 7) valid positive proofs; targets come straight from the local routing table and keys are dynamically batched by peer.
...
1. For each locally stored key `K`, recompute retention responsibility (storage-retention width) using current `SelfInclusiveRT(self)`:
   ...
   b. If out of range: if `RecordOutOfRangeFirstSeen(self, K)` is `None`, set it to `now` — immediately, even if a retained commitment currently contains `K`. Once hysteresis elapses, classify `K` against the current widest-admission group. If a complete width-20 group excludes self, delete without a remote proof after immediate bootstrap, width-9, width-20, hysteresis, and commitment revalidation. Otherwise group a bounded rotating window of candidates by strict-close peer, split requests at the dynamic square-root key limit, and require 6-of-7 positive nonce-bound possession proofs. Bound work by actual requests and complete candidate proof sets; grade a responder's size rejection as an attributable failure (distinct `size_reject` label) rather than renegotiating it in-band, relying on the responder's 2x acceptance margin and the assumed ~2x close-group store spread. Immediately before audited deletion, recheck bootstrap, width-9 responsibility, hysteresis, commitment retention, current strict-group membership, and proof membership. Anything deferred or below threshold retains the record, timestamp, and candidacy. Trust penalties for genuine failed proofs are emitted only after fresh responsibility confirmation; first-time bootstrap claims are penalized only after the bootstrap grace period, while repeated claims are penalized immediately.
...
2. Challenger selects one peer uniformly at random from peers with peer-level sync history sufficient for `RepairOpportunity` as `challenged_peer_id`. If no eligible peers exist, the audit tick is idle.
...
4. For each `K` in `SeedKeys`, challenger checks whether `challenged_peer_id` appears in the `CLOSE_GROUP_SIZE` closest peers for `K` via local RT lookup and whether a mature `RepairProof(challenged_peer_id, K)` exists for that exact close-group snapshot. If the current snapshot differs from the snapshot recorded with the proof, all local proofs for `K` are invalidated. Keys where the peer is not responsible or where no key-specific mature repair proof exists are discarded. The remaining keys form `PeerKeySet(challenged_peer_id)`.
...
- When a full neighbor-sync round-robin cycle completes, node scans the local store using the current self-inclusive routing view. Keys inside width 9 clear `RecordOutOfRangeFirstSeen`; keys outside width 9 start or continue the process-local hysteresis even while commitment-held. Once mature, a key outside a complete width-20 group is fast-deleted without remote proof after immediate local revalidation; this intentionally accepts the last-copy risk. Other mature keys are grouped by strict-close target peer into dynamically sized multi-key challenges and require 6-of-7 positive nonce-bound proofs. Candidate and actual-request budgets are bounded and rotating; partial candidate proof sets are not scheduled, and a size rejection grades as an attributable failure (`size_reject`) rather than being split and retried, since honestly-sized peers do not reject within the assumed store spread. Any bootstrap, commitment, budget, topology, or proof deferral preserves timestamp and candidacy. Paid-list timing and confirmation remain independent.
...
- Key `K` goes out of width 9 at time `T`; the timestamp starts immediately and deletion before three days is impossible. At maturity, a complete width-20 view excluding self takes the locally revalidated fast path with no audit; otherwise deletion still requires 6-of-7 current strict-group proofs.
