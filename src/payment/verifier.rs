...
///   tracking spans a wider group than storage.
///
...
pub struct PaymentVerifier {
    ...
    /// Test-only override for the paid-quote known-peer check.
    ///
    /// Production code derives closest peers from the attached [`P2PNode`].
    #[cfg(any(test, feature = "test-utils"))]
    test_paid_quote_known_peers_override: RwLock<Option<Vec<[u8; 32]>>>,
    ...
    /// Test-only override for receiver-membership checks.
    ///
    /// Production derives membership from the attached [`P2PNode`].
    #[cfg(any(test, feature = "test-utils"))]
    test_receiver_membership_override: RwLock<Option<bool>>,
    /// Configuration.
...
impl PaymentVerifier {
    ...
    pub fn new(config: PaymentVerifierConfig) -> Self {
        ...
        Self {
            ...
            #[cfg(any(test, feature = "test-utils"))]
            test_paid_quote_known_peers_override: RwLock::new(None),
            #[cfg(any(test, feature = "test-utils"))]
            test_completed_payments_override: RwLock::new(HashMap::new()),
            #[cfg(any(test, feature = "test-utils"))]
            test_receiver_membership_override: RwLock::new(None),
            config,
        }
    }
    ...
    pub fn set_paid_quote_known_peers_for_tests(&self, peer_ids: Vec<[u8; 32]>) {
        *self.test_paid_quote_known_peers_override.write() = Some(peer_ids);
    }
    ...
    /// Test-only setter for receiver-membership checks.
    #[cfg(any(test, feature = "test-utils"))]
    pub fn set_receiver_membership_for_tests(&self, is_member: bool) {
        *self.test_receiver_membership_override.write() = Some(is_member);
    }
    ...
    ///   [`VerificationContext`] for receiver-membership width
    ///
    /// # Returns
    ///
    ...
    pub async fn verify_payment(
        ...
        self.validate_receiver_membership(xorname, context).await?;
    ...
    async fn validate_receiver_membership(
        &self,
        xorname: &XorName,
        context: VerificationContext,
    ) -> Result<()> {
        ...
        #[cfg(any(test, feature = "test-utils"))]
        {
            let membership_override = *self.test_receiver_membership_override.read();
            if let Some(is_member) = membership_override {
                if is_member {
                    return Ok(());
                }
                return Err(Error::Payment(format!(
                    "{context:?} receiver is not in the required local peer set for key {}",
                    hex::encode(xorname)
                )));
            }
        }
        ...
        let Some(p2p_node) = attached else {
            ...
            {
                crate::logging::warn!(
                    "PaymentVerifier: no P2PNode attached; {context:?} receiver \
                     membership check SKIPPED (test build). Production startup \
                     MUST call PaymentVerifier::attach_p2p_node."
                );
                return Ok(());
            }
            #[cfg(not(any(test, feature = "test-utils")))]
            {
                ...
                return Err(Error::Payment(format!(
                    "{context:?} rejected: verifier is not wired to the P2P \
                         layer; cannot verify receiver membership."
                )));
            }
        };
        ...
        let closest = p2p_node
            .dht_manager()
            .find_closest_nodes_local_with_self(xorname, width)
            .await;
        ...
        Err(Error::Payment(format!(
            "{context:?} receiver {} is not among this node's local {width} closest peers for {}",
            self_id.to_hex(),
            hex::encode(xorname)
        )))
    }
    ...
    /// 4. Each candidate must also come from a locally known close peer and
    ...
    /// quote's content hash, quote hash, signature, local floor, known-peer
    ...
    async fn verify_legacy_median_candidate(
        ...
        self.validate_paid_quote_known_peer(xorname, &issuer_peer_id)
            .await?;
    ...
    async fn validate_paid_quote_known_peer(
        &self,
        xorname: &XorName,
        issuer_peer_id: &PeerId,
    ) -> Result<()> {
        #[cfg(any(test, feature = "test-utils"))]
        if let Some(known_peer_ids) = self.test_paid_quote_known_peers_override.read().as_ref() {
            if known_peer_ids
                .iter()
            ...
            return Err(Error::Payment(format!(
                "Paid quote issuer {} is not among this node's local {K_BUCKET_SIZE} closest peers for {}",
        ...
        let Some(p2p_node) = attached else {
            ...
            {
                crate::logging::warn!(
                    "PaymentVerifier: no P2PNode attached; paid-quote known-peer \
                     check SKIPPED (test build). Production startup MUST call \
        ...
        let closest = p2p_node
            .dht_manager()
            .find_closest_nodes_local_with_self(xorname, K_BUCKET_SIZE)
            .await;
...
mod tests {
    ...
    fn create_test_verifier() -> PaymentVerifier {
        ...
        verifier.set_receiver_membership_for_tests(true);
        verifier
    }
    ...
    fn mark_known_paid_candidates(
        ...
        let known_peers = median_test_candidates(peer_quotes)
            .iter()
            .map(|(peer_id, _)| *peer_id.as_bytes())
            .collect();
        verifier.set_paid_quote_known_peers_for_tests(known_peers);
    }
    ...
    #[tokio::test]
    async fn test_client_put_receiver_membership_runs_before_cache() {
        let verifier = create_test_verifier();
        let xorname = [0xB4u8; 32];
        verifier.cache.insert(xorname);
        verifier.set_receiver_membership_for_tests(false);
        ...
        assert!(
            ...
            "Error should mention receiver membership: {err}"
        );
    }

    #[tokio::test]
    async fn test_paid_list_receiver_membership_runs_before_cache() {
        let verifier = create_test_verifier();
        let xorname = [0xB7u8; 32];
        verifier.cache.insert_paid_list_verified(xorname);
        verifier.set_receiver_membership_for_tests(false);

        let err = verifier
            .verify_payment(&xorname, None, VerificationContext::PaidListAdmission)
            .await
            .expect_err("cached paid-list admission must still reject non-members");
    ...
    async fn test_legacy_paid_median_full_path_accepted() {
        ...
        mark_known_paid_candidates(&verifier, &peer_quotes);
    ...
    #[tokio::test]
    async fn test_legacy_paid_median_known_peer_rejection() {
        let verifier = create_test_verifier();
        verifier.set_records_stored_for_tests(0);
        verifier.set_paid_quote_known_peers_for_tests(vec![rand::random()]);
        ...
        let err = verifier
            .verify_payment(&xorname, Some(&proof_bytes), VerificationContext::ClientPut)
            .await
            .expect_err("unknown paid issuer should be rejected");

        assert!(
            format!("{err}").contains("not among this node's local"),
            "Error should mention local closest peers: {err}"
        );
    }
    ...
    #[tokio::test]
    async fn test_legacy_paid_list_admission_enforces_known_peer() {
        let verifier = create_test_verifier();
        verifier.set_records_stored_for_tests(0);
        verifier.set_paid_quote_known_peers_for_tests(Vec::new());
        ...
        let err = verifier
            ...
            .await
            .expect_err("paid-list admission must enforce the paid issuer known-peer check");
