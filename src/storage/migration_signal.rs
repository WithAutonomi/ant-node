...
///
/// Often enough that a reading is never many hours stale, rarely enough that it is a line
/// an operator can read rather than a stream. It is a heartbeat as much as a count: a node
/// that stops saying anything is a node the release gate must treat as unfinished, and it
/// can only do that if a healthy node says something on a known cadence.
...
pub struct PeerTally {
    /// Peers that still have an old chunk store.
    pub legacy: usize,
    /// Peers that have finished.
    pub files: usize,
    /// Peers that could not tell, or that answered with something this build does not know.
    pub unknown: usize,
...
pub async fn tally_peers(p2p: &Arc<P2PNode>) -> PeerTally {
    ...
    for peer in transport.connected_peers().await {
        ...
        //
        // Only for peers that are NOT finished. The gate asks which distinct nodes are still
        // outstanding, so those are the ones that need naming; a fleet that has finished
        // emits none of these at all, and the cost falls away as the answer arrives rather
        // than peaking when it does. The aggregate line below is emitted either way and
...
/// Say where this node is, and what it can see, until it shuts down.
///
/// Structured rather than prose, because the thing that reads it is a query and not a
/// person: a release decision is "no node says `legacy`, and every node we expect to hear
/// from said something recently", and neither half of that can be answered from a sentence.
pub async fn report_until_shutdown(
    ...
    loop {
        // Wait first, report second. A node that has just started has no peers to describe
        // and nothing has changed on its disk since the user agent was built from it, so the
        // first pass would be pure startup cost on the busiest moment in a node's life.
        tokio::select! {
            () = shutdown.cancelled() => return,
            () = tokio::time::sleep(REPORT_INTERVAL) => {}
        }
        // A **weak** handle, upgraded per pass and dropped again before the sleep. This task
        // observes a node; it must never be the reason one stays alive. Holding a strong one
        // keeps the node, and with it its bound transport, for as long as this task runs, so
        // any path that drops a node without cancelling its token would leak a live port
        // rather than a stopped node. Nothing is left to notice that but the next thing that
        // fails to bind.
        let Some(p2p) = p2p.upgrade() else {
            return;
        };
        // Re-read the disk every time rather than reusing what the user agent was built
        // from. The agent is fixed when the transport is built; this is not, so a node that
        // finishes migrating says so here within the interval instead of at its next
        // restart.
        let own = MigrationSignal::from_disk(&root_dir);
        let peers = tally_peers(&p2p).await;
        if own == MigrationSignal::Legacy || own == MigrationSignal::Unknown {
            warn!(
                ...
                "This node still has an old chunk store ({}). {} of the {} node(s) it can \
                 see are not finished either.",
                own.token(),
                peers.outstanding(),
                peers.outstanding() + peers.files
            );
        } else {
            info!(
                migration_event = "signal",
                ...
                peers_unreported = peers.unreported,
                peers_files = peers.files,
                "Storage migration: this node is done; {} of the {} node(s) it can see are \
                 not.",
                peers.outstanding(),
                peers.outstanding() + peers.files
            );
        }
        // Dropped before the next wait, so the node is not held across the interval.
        drop(p2p);
    }
}
