...
impl AntProtocol {
    ...
    pub(crate) async fn try_handle_request_with_context(
        ...
        // choke point. Only this entry point has the wire length; the browser
        // adapter below is handed an already-decoded message.
        traffic::record_rx(ChunkRequestKind::of(&message.body), data.len());

        let (response, mut get_telemetry, traffic_key) =
    ...
    async fn handle_message_with_context(
        ...
        Option<ChunkMessage>,
        Option<GetRequestTelemetry>,
        Option<ChunkResponseKey>,
    ) {
