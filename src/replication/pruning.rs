...
use crate::replication::config::{
    ...
    AUDIT_FAILURE_TRUST_WEIGHT, GRACE_POSSESSION_AUDIT_TIMEOUTS, MAX_FAST_PRUNE_DELETIONS_PER_PASS,
    MAX_PRUNE_AUDIT_CANDIDATES_PER_PASS, MAX_PRUNE_AUDIT_REQUESTS_PER_PASS,
};
...
async fn receive_prune_audit_response(
    ...
    .await;
    let (failure_class, send_failure) = match result {
        ...
        PruneAuditChallengeResult::NoResponse(class) => {
            ...
            (class.as_str(), PruneAuditSendFailure::Unanswered)
        }
        PruneAuditChallengeResult::MalformedResponse => {
            ("malformed", PruneAuditSendFailure::Malformed)
        }
    };

    // The possession-audit protocol move makes silence ambiguous during the
    // rollout. An actual reply that fails to decode remains attributable.
    if !prune_send_failure_is_penalised(send_failure) {
        debug!(
...
async fn send_prune_audit_challenge(
    ...
    match ReplicationMessage::decode_audit_response(&response.data) {
        Ok(msg) => PruneAuditChallengeResult::Response(Box::new(msg)),
        Err(e) => {
